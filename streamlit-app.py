...
def paginate_table(table_data):
    ...
    with top_menu[0]:
        sort = st.radio("Sort Data", options=["Yes", "No"], horizontal=1, index=1, key="sort_radio")
    if sort == "Yes":
        with top_menu[1]:
            sort_field = st.selectbox("Sort By", options=table_data.columns, key="sort_field_select")
        with top_menu[2]:
            sort_direction = st.radio(
                "Direction", options=["⬆️", "⬇️"], horizontal=True, key="sort_direction_radio"
            )
    ...
    with bottom_menu[2]:
        batch_size = st.selectbox("Page Size", options=[10, 25, 50, 100], key="page_size_select")
    with bottom_menu[1]:
        total_pages = max(int(len(table_data) / batch_size), 1)
        current_page = st.number_input(
            "Page", min_value=1, max_value=total_pages, step=1, key="page_number_input"
        )
    with bottom_menu[0]:
